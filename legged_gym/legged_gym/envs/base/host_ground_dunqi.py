...
class LeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        ...
        if self.cfg.domain_rand.randomize_com_displacement:
            ...
            self.com_displacement[:, 0] = self.com_displacement[:, 0] * 4 - 0.3
    ...
    def _reward_shoulder_roll_deviation(self):
        ...
        reward = (self.dof_pos[:, self.shoulder_roll_joint_indices[0]] < -1.2) | (self.dof_pos[:, self.shoulder_roll_joint_indices[1]] > 1.2)
        return reward
    ...
    def _reward_knee_deviation(self):
        ...
        reward = (torch.max(torch.abs(self.dof_pos[:, self.knee_joint_indices]), dim=-1)[0] > 2.5) | (torch.min(self.dof_pos[:, self.knee_joint_indices], dim=-1)[0] < 0.01)
        return reward
    ...
    def _reward_yaw_torque(self):
        reward = torch.max(torch.abs(self.torques[:, [2,8]]), dim=-1)[0] < 70
        return reward
    
    def _reward_torque_balance(self):
        # 提取左右髋关节力矩: shape (num_envs, 3)
        left_hip_torques = self.torques[:, [0, 1, 2, 3]] * 0.01  # shape: (num_envs, 3)
        right_hip_torques = self.torques[:, [6, 7, 8, 9]] * 0.01 # shape: (num_envs, 3)
        left_hip_torques[:,2] *= 2
        right_hip_torques[:,2] *= 2
        ...
        under_standup  = self.root_states[:, 2] < self.cfg.rewards.target_base_height_phase1

        return combined_var*under_standup
    ...
    def _reward_ground_parallel2(self):
        left_ankle_pos = self.rigid_body_states[:, self.left_ankle_indices, 2].clone() * 10
        right_ankle_pos = self.rigid_body_states[:, self.right_ankle_indices, 2].clone() * 10
        var = left_ankle_pos.var(1) + right_ankle_pos.var(1)
        var = torch.mean(torch.concat([left_ankle_pos.var(1).view(-1, 1), right_ankle_pos.var(1).view(-1, 1)], dim=-1), dim=-1)
